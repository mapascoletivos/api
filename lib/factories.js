...
var assignContentsForFeature = function(feature, doneAssign) {
	
	var contentsArray = [];
	
	async.times(2, function(n,done){
		// create a content
		Factory.create('content', {creator: feature.creator, feature: feature}, function(content){
			done(null,content);
		});
...
var assignFeaturesForLayer = function(layer, doneAssign) {
	...
	async.times(10, function(n,done){
		...
		Factory.create('feature', {creator: layer.creator, layer: layer}, function(feature){
			assignContentsForFeature(feature, done);
			// done(null,feature);
		});
