...
exports.acceptInvitation = function (req, res) {
  ...
  if (token.type != 'acceptInvitation') {
    ...
    mongoose
      .model('User')
      .findOne({ email: token.data.user.email }, function (err, user) {
        if (err) {
          return res.render('tokens/index', {
            errors: [
              req.i18n.t('token.accept_invitation.error.user_activation')
            ],
            callbackUrl: token.callbackUrl,
            autoRedirect: false
          });
...
exports.newPassword = function (req, res) {
  ...
  if (token.type != 'password_reset' && token.type != 'password_needed') {
    ...
    mongoose.model('User').findById(token.user, function (err, user) {
      if (err) {
        return res.render('tokens/index', {
          errors: [req.i18n.t('token.new_password.error.generic')]
        });
      }
